        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body pt-2">
                    ...
                    <form action="{{ route('local_city.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-lg-6">
                                    ...
                                    <tbody>
                                        @foreach ($items as $row)
                                            <tr>
                                                ...
                                                @php
                                                    $data = get_local_price($row->id, $item->name);
                                                @endphp
                                                @if ($data != null)
                                                    <input type="hidden" value="{{ $data->id }}" name="data_id[]">
                                                    <td><input type="number" class="form-control" name="eight_hr[]"
                                                            value="{{ $data->eight_hr }}">
                                                    </td>
                                                    <td><input type="number" class="form-control" name="twelve_hr[]"
                                                            value="{{ $data->twelve_hr }}">
                                                    </td>
                                                    <td><input type="number" class="form-control" name="extra_hr[]"
                                                            value="{{ $data->extra_hr }}">
                                                    </td>
                                                @else
